...
cb = plt.colorbar(mm[0],orientation='vertical',ax=ax)
...
fig, ax = plt.subplots(1,1,figsize=(15,10))
ax.set_aspect('equal')
mm = mesh.plotSlice((inversion_model_grav),normal='Z', ind=-5,clim=[-.2,.2], grid=False,pcolorOpts={'cmap':'seismic'},ax=ax,norm=MidpointNormalize(midpoint=0))

plot_true_geology(indslicezplot,1)
##
geocb = plt.colorbar(mm[0],norm = MidpointNormalize(midpoint=0))
...
cb = plt.colorbar(mm[0],orientation='horizontal',ax=ax)
cb.set_label('Density', fontsize=18)
mm = mesh.plotSlice((inversion_model_grav),normal='Y', grid=False,clim = [-.2,.2],pcolorOpts={'cmap':'seismic'},ax=ax)
plot_true_geology(indsliceyplot,2)
